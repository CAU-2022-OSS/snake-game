...
class Window:
            ...
            while self.running:
                for event in pg.event.get():
                    if event.type == pg.MOUSEBUTTONUP:
                        mouse = pg.mouse.get_pos()
                        if (
                            menu[2] + menu[0] > mouse[0] > menu[0]
                            and menu[1] + menu[3] > mouse[1] > menu[1]
                            and menu[4] != None
                        ):
    ...
    def ranking(self, player=None, apple=None):
        ...
        if len(self.rank) > 0:
            for i in range(len(self.rank)):
                ...
                self.draw_text(
                    str(self.rank[i][1]), 22, BLACK, WIDTH * 3 / 4 - 50, HEIGHT / 3 + (30 * (i + 1))
                )  # score
    ...
    def show_single_game_screen(self, player=None, apple=None):
        ...
        while self.running:
            ...
            if (
                player.positions[0][0] < 5
                or player.positions[0][0] > 42
                or player.positions[0][1] < 3
                or player.positions[0][1] > 40
            ):
    ...
    def show_dual_game_screen(self, player1=None, player2=None, apple1=None, apple2=None):
        ...
        while self.running:
            ...
            for i in appleList:
                if player1.positions[0] == i.position:
                    ...
                    while i.position in (
                        player1.positions,
                        player2.positions,
                    ):  #  while new apple's position overlaps with snake
            ...
            # event) collision with wall
            if (
                player1.positions[0][0] < 12
                or player1.positions[0][0] > 51
                or player1.positions[0][1] < 3
                or player1.positions[0][1] > 81
            ):
                player1.is_dead = 1
                self.dual_game_over_screen(player1, player2, apple1, apple2)
                break
            if (
                player2.positions[0][0] < 12
                or player2.positions[0][0] > 51
                or player2.positions[0][1] < 3
                or player2.positions[0][1] > 81
            ):
                player2.is_dead = 1
                self.dual_game_over_screen(player1, player2, apple1, apple2)
                break
    ...
    def auto_move(self, player=None, apple=None):
        ...
        if (
            x != 40
            and y >= 6
            and ((5, 40) not in player.positions[1:] or x + 2 < player.positions[len(player.positions) - 1][1])
            and (
                x + 1 < apple.position[1]
                or x > apple.position[1]
                or (apple.position[0] == y and apple.position[1] - 1 == x)
            )
        ):
            next = "E"
        else:
            if priority[y][x] + 1 == priority[y][x + 1]:
                next = "E"
            elif priority[y][x] + 1 == priority[y][x - 1]:
                next = "W"
            elif priority[y][x] + 1 == priority[y + 1][x]:
                next = "S"
            else:
                next = "N"
