...
class Window:
    ...
    def handle_dir1(self, player, event):
        if event.key == pg.K_w and player.direction != 'Ss':
            player.direction = KEY_DIRECTION1[event.key]
        elif event.key == pg.K_s and player.direction != 'Nw':
            player.direction = KEY_DIRECTION1[event.key]
        elif event.key == pg.K_a and player.direction != 'Ed':
            player.direction = KEY_DIRECTION1[event.key]
        elif event.key == pg.K_d and player.direction != 'Wa':
            player.direction = KEY_DIRECTION1[event.key]

    def handle_dir2(self, player, event):
        if event.key == pg.K_UP and player.direction != 'S':
            player.direction = KEY_DIRECTION2[event.key]
        elif event.key == pg.K_DOWN and player.direction != 'N':
            player.direction = KEY_DIRECTION2[event.key]
        elif event.key == pg.K_LEFT and player.direction != 'E':
            player.direction = KEY_DIRECTION2[event.key]
        elif event.key == pg.K_RIGHT and player.direction != 'W':
            player.direction = KEY_DIRECTION2[event.key]
    ...
    def show_dual_game_screen(self, player1=None, player2=None, apple1=None, apple2=None):
        ...
        while self.running:
            ...
            for event in pg.event.get():
                if event.type == pg.KEYDOWN:
                    self.handle_dir1(player1, event)
                    self.handle_dir2(player2, event)
                    if event.key==pg.K_ESCAPE:
